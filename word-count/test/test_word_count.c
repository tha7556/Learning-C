                           ...
                           int actual_word_count)
{
   ...
   for (int index = 0; index < MAX_WORDS; index++) {
	   printf("actual solution[%d]: %s(%d) vs %s(%d)\n",index, actual_solution[index].text, actual_solution[index].count, expected_solution[index].text, expected_solution[index].count);
