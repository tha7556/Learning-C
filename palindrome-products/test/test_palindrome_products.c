...
void check_factors(factor_t * actual, int depth, factor_t expected[])
{
   ...
   for (i = 0; i < depth; i++) {
      if (actual == NULL)
         break;
      if ((actual->factor_a == expected[i].factor_a) && (actual->factor_b == expected[i].factor_b))
         count_ok++;
	 else if ((actual->factor_b == expected[i].factor_a) && (actual->factor_a == expected[i].factor_b))
         count_ok++;
      actual = actual->next;
   }
   TEST_ASSERT_EQUAL_PTR(NULL, actual);
